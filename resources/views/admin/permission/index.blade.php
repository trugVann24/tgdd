<x-app-layout>
    <div class="flex items-center justify-between border-b border-gray-700 pb-3">
        <div class="">
            {{ __('Danh sách Permission') }}
        </div>
        <div class="">
            <a href="{{ route('admin.permissions.create') }}"
                class="bg-indigo-700 px-3 py-2 rounded-sm font-inter-500 text-sm hover:bg-indigo-800">Thêm permission</a>
        </div>
    </div>
                                ...
                                <tr class="">
                                    ...
                                    <td class="px-6 py-4 flex items-center justify-end font-inter-500 ">
                                        <div
                                            ...
                                            <a href="{{route('admin.permissions.edit', $permission->id)}}" class="">
                                                <i class='bx bx-edit text-lg'></i>
                                            </a>
                                        </div>
                                        <form action="{{route('admin.permissions.destroy', $permission->id)}}" method="POST" onsubmit="return confirm('Bạn có chắc chắn muốn xoá ?')">
                                            @csrf
                                            @method('DELETE')
