<x-app-layout>
    {{ __('Thêm chi tiết hoá đơn') }}
    <div class="w-full mx-auto p-5 rounded-md border dark:border-gray-700">
        <form method="POST" action="{{ route('admin.invoicedetail.store') }}">
            @csrf           
            <div>
                <x-input-label for="code_bill" :value="__('Mã hoá đơn')" />
                <x-text-input id="code_bill" class="block mt-1 w-full" type="text" name="code_bill" :value="old('code_bill')"
                    required autofocus />
            ...
            <div class="mt-4">
             ...
             <div class="mt-4">
                ...
                <x-text-input id="quantity" class="block mt-1 w-full" type="number" name="quantity" :value="old('quantity')" required autofocus oninput="calculateTotalMoney()" />
                <x-input-error :messages="$errors->get('quantity')" class="mt-2" />
            </div>
            
            <div class="mt-4">
                <x-input-label for="price" :value="__('Giá ($)')" />
                <x-text-input id="price" class="block mt-1 w-full" type="text" name="price" :value="old('price')" required autofocus readonly oninput="calculateTotalMoney()" />
            ...
            <div class="mt-4">
                ...
                <x-text-input id="total_money" class="block mt-1 w-full" type="text" name="total_money" :value="old('total_money')" required autofocus readonly />
