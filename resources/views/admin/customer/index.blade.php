                ...
                <div class="overflow-hidden">
                    <table table="myTable" class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                        <thead>
                            <tr>
                                <th scope="col"
                                    class="px-6 py-4 text-start text-xs font-inter-500 text-gray-500 uppercase">ID</th>
                                <th scope="col"
                                    class="px-6 py-4 text-start text-xs font-inter-500 text-gray-500 uppercase">Tên Khách hàng
                                </th>
                                <th scope="col"
                            ...
                            @foreach ($list as $key => $staf)
                                <tr class="">
                                    ...
                                    <td class="px-6 py-4 text-end text-xm text-sm text-gray-800 dark:text-gray-200">
                                        {{ \Carbon\Carbon::parse($staf->day_of_birth)->format('d/m/Y') }}</td>
                                    </td>
                                    <td class="px-6 py-4 text-end text-xm text-sm text-gray-800 dark:text-gray-200">
                                        {{ $staf->revenue }}</td>
                                    </td>
                                    ...
                                    <td class="px-6 py-4 flex items-center justify-end font-inter-500 ">
                                        <div
                                            ...
                                            <a href="{{ route('customer.edit', $staf->id) }}" class="">
                                                <i class='bx bx-edit text-lg'></i>
                                            </a>
                                        </div>
                                        <form action="{{ route('customer.destroy', $staf->id) }}" method="POST"
